...
from mmgbsa.config import PROTEIN_FF, STANDARD_AMINO_ACIDS
...
def validate_protein_only(pdb_path: Path) -> None:
    ...
    for model in structure:
        for chain in model:
            for residue in chain:
                ...
                # Skip hetero flags for waters/ions
                if residue.id[0] not in [' ', 'H_MSE', 'H_M3L', 'H_CAS']:
    ...
    if non_protein_residues:
        ...
        error_lines.extend([
            ...
            "",
            "For ligand parameterization, use: mmgbsa parameterize",
        ])
