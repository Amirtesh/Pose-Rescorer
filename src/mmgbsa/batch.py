...
def process_single_ligand(
    ...
    try:
        ...
        return {
            ...
            "delta_g_gb": energies.get("DELTA_G_GB"),
            "status": "success",
        }
...
def run_batch_mmgbsa(
    ...
    """
    logger.info("Starting batch MM/GBSA rescoring")
    ...
    with open(csv_path, "w", newline="") as csvfile:
        fieldnames = ["ligand_name", "delta_g_bind", "delta_h", "delta_g_gb", "status"]
    ...
    table.add_column("ΔG_GB", justify="right")
    ...
    for result in sorted_results + failed_results:
        ...
        dgb = f"{result['delta_g_gb']:.2f}" if result['delta_g_gb'] is not None else "N/A"
        table.add_row(
            result["ligand_name"],
            dg,
            dh,
